...
public class Point
{
    ...
    public int idSpecalny = -1; // -1 -> Nejedn� se o spec�ln�
...
public class LevelCotroller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            // Jsme na konci �seku ?et�zce?
            if (buttons[i] == null)
            {
                pre = null;
                continue;
            }
            // M?me ji� toto tla�?tko zaregistrovan??
            Point p = null;
            for (int j = 0; j < points.Count; j++)
            {
                if (buttons[i] == ((Point)points[j]).button)
                {
                    p = (Point)points[j];
                    break;
                    // Ano, m?me a odkaz je v prom�nn? p
                }
            }
            // Kdy� nem?me, p?id?me do seznamu
            if (p == null)
            {
        ...
        // Z�sk�n� dat pro ukl�d�n� a na?�t�n� levelu
        if(autoSave){
    ...
    public void save(){
        ...
        if(autoSave){
            ...
            // -------------    Data, kter� se budou ukl�dat
            // N�zev levelu, ve kter�m se hr�? nyn� nach�z�
    ...
    public void load(){
        ...
        if(autoSave && System.IO.File.Exists(Application.persistentDataPath+"/"+ GlobalVariables.savedirectoryName + "/" + GlobalVariables.saveName + ".bin")){
            ...
            try{
                ...
                print("Chyba, nesed� seznam pohybliv�ch ?�st�");
            }
            try{
                ...
                print("Chyba, nesed� seznam ?�st� k destrukci");
            }
